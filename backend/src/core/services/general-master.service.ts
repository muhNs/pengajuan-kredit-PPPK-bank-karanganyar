...
const executeDbOperation = async (
  ...
  switch (category) {
    case "status-rumah":
      ...
      if (action === "create")
        return await prisma.statusRumah.create({
          data: { kepemilikan: data.nama },
        });
      if (action === "update")
        return await prisma.statusRumah.update({
          where: { id: data.id },
          data: { kepemilikan: data.nama },
        });
      if (action === "delete")
        return await prisma.statusRumah.delete({ where: { id: data.id } });
      break;

    case "status-pernikahan":
      if (action === "find") return await prisma.statusPernikahan.findMany();
      if (action === "create")
        return await prisma.statusPernikahan.create({
          data: { status: data.nama },
        });
      if (action === "update")
        return await prisma.statusPernikahan.update({
          where: { id: data.id },
          data: { status: data.nama },
        });
      if (action === "delete")
        return await prisma.statusPernikahan.delete({ where: { id: data.id } });
      break;

    case "jenis-kelamin":
      if (action === "find") return await prisma.jenisKelamin.findMany();
      if (action === "create")
        return await prisma.jenisKelamin.create({
          data: { gender: data.nama },
        });
      if (action === "update")
        return await prisma.jenisKelamin.update({
          where: { id: data.id },
          data: { gender: data.nama },
        });
      if (action === "delete")
        return await prisma.jenisKelamin.delete({ where: { id: data.id } });
      break;

    case "hubungan-penjamin":
      if (action === "find") return await prisma.hubunganPenjamin.findMany();
      if (action === "create")
        return await prisma.hubunganPenjamin.create({
          data: { status: data.nama },
        });
      if (action === "update")
        return await prisma.hubunganPenjamin.update({
          where: { id: data.id },
          data: { status: data.nama },
        });
      if (action === "delete")
        return await prisma.hubunganPenjamin.delete({ where: { id: data.id } });
      break;
...
export const updateData = async (
  ...
  return await executeDbOperation(category, "update", {
    id,
    nama: payload.nama,
  });
};
